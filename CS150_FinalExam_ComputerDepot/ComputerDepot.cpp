...
* i.  Case 1 (Add Item):
* (Prompts and logic for adding Computer or Laptop)
* ii. Case 2 (Display Inventory):
* (Prints inventory header and items)
* iii.Case 3 (Quit):
* Sets flag to exit loop.
* iv. Default (Invalid choice):
...
void pauseExecution() {
    ...
    // cin.ignore() might be needed here if there are leftover newlines from previous cin >> var
    // but clearInputBuffer() is generally called before prompts that might need it.
    // For a simple pause, cin.get() is often sufficient after a full line has been read by getline
    // or after cin.ignore() has cleaned up the buffer from a cin >> operation.
    // If cin.get() directly follows a cin >> operation without a cin.ignore(),
    // it might consume the leftover newline.
    // Let's ensure the buffer is clear before cin.get() for pause.
...
int main(int argc, char* argv[])
{
    ...
    while (running) {
        ...
        default: // Should not be reached due to initial input validation
